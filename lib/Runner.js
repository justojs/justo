...
//internal data
var loggers, reporters, automator, tester, only;
...
export default class Runner {
  ...
  static runTesterWork(work) {
    ...
    for (let entry of work.src) {
      if (!fs.exists(entry)) throw new Error(`The '${entry}' entry doesn't exist.`);
    }

    //(2) run
    if (!tester) tester = new Tester({loggers, reporters, only});
    ...
    try {
      tester.workflow(work.name, function() {
        for (let src of work.src) {
          let entry = fs.entry(process.cwd(), src);

          if (entry instanceof fs.File) requireFile(entry);
          else if (entry instanceof fs.Dir) requireDir(entry);
